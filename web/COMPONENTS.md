...
|-----------|----------|---------|
| AppShell | `ConnectionSwitcher`, `Lamp`, `ThemeToggle`, `KeeperMark`, `Tooltip`, `Button` | The persistent chrome: the mark, the connection scope control, the six sections (Connections, Approvals, Permissions, Activity, Catalog, Policy), the daemon's `Lamp`, the theme toggle, and settings as the right-most control. The current section is a foreground rule that grows from centre, never the amber accent (UI.md §1's governing rule) |
| ConnectionSwitcher | `DropdownMenu`, `Button` | The connection scope control. Lists every registered connection via `listConnections()`; a degraded connection (accepted G0 findings, SPEC R4.1) carries its ⚠ marker here, the same meaning it carries on `Approvals` (UI.md §2.5) |
...
|-----------|-------|-------|
| ConnectionsPage | `/connections` | Registering a database, the G0 findings report, accepting findings by name (SPEC §4.1) |
...
| ApprovalsPage | `Table`, `Lamp`, `Facts`, `Button`, `Empty` | The shared queue across every agent session (SPEC R9.1). Oldest first, never reordered; each row names its client, workspace and stated intent; deciding one item does not advance to the next |
| ApprovalDetail | `Facts`, `Separator`, `Button` | One queued item: §9.2's facts above its SQL, the accepted-findings warning where it applies, and a write's "revocation restores nothing" line where it applies and nowhere else |
| AcceptedFindingsMark | none | The `(!)` beside a connection running with accepted G0 findings (SPEC R4.1). The only amber on the queue other than the waiting lamp, carrying the same meaning: this needs you to know something |
...
| ConnectionsPage | `Table`, `Lamp`, `Facts`, `Checkbox`, `Input`, `Button` | Register a database and accept its privilege findings (SPEC R4.1). keeper never refuses a credential; a master account works, and what this screen does is make it impossible to hold one by accident |
| ConnectionDetailPanel | `Facts`, `Table`, `Separator`, `Button` | One connection: what it is, what the audit found, and what has been accepted, with dates and actors |
| FindingsAcceptance | `Checkbox`, `Input`, `Button` | One checkbox per finding, never a single "I understand the risks" over a collapsed list — that is the control people click without reading, and the requirement exists to prevent it. Each finding shows its one-sentence meaning and the narrower grant that would remove it, copyable |
| RegisterForm | `Input`, `Button` | Registering a database. The connection is stored disabled; the findings are what enables it |
